...
class MyConsensusForecastsItem(ConsensusForecastsItem):
    """Дополненный параметрами класс ConsensusForecastsItem."""
    def __init__(self, consensus: ConsensusForecastsItem, instrument_uid: str, ticker: str):
        super().__init__(uid=consensus.uid, asset_uid=consensus.asset_uid, created_at=consensus.created_at,
                         best_target_price=consensus.best_target_price, best_target_low=consensus.best_target_low,
                         best_target_high=consensus.best_target_high, total_buy_recommend=consensus.total_buy_recommend,
                         total_hold_recommend=consensus.total_hold_recommend,
                         total_sell_recommend=consensus.total_sell_recommend, currency=consensus.currency,
                         consensus=consensus.consensus, prognosis_date=consensus.prognosis_date)
        self.instrument_uid: str = instrument_uid
...
def print_function_runtime(decorated_function):
    def wrapper_function(*args):
        ...
        print('Время выполнения функции \"{0}\": {1}c.'.format(decorated_function.__name__, delta))
        return result
