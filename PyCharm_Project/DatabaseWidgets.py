...
class ComboBox_Status(QtWidgets.QComboBox):
    ...
    class TokenStatusesModel(QtCore.QAbstractListModel):
        ...
        __ANY_STATUS: str = 'Любой'
        __PARAMETER: str = 'status'
            ...
            if self.__token is not None:
                ...
                if db.transaction():
                    ...
                    while statuses_query.next():
                        self.__instrument_statuses.append(statuses_query.value(self.__PARAMETER))
        ...
        def data(self, index: QtCore.QModelIndex, role: int = ...) -> typing.Any:
            if role == QtCore.Qt.ItemDataRole.DisplayRole:
                row: int = index.row()
                return self.__ANY_STATUS if row == 0 else self.__instrument_statuses[row - 1]
    ...
    def __init__(self, token: TokenClass | None = None, parent: QtWidgets.QWidget | None = None):
        ...
        self.__token_statuses_model = self.TokenStatusesModel(token=token, parent=self)
        self.setModel(self.__token_statuses_model)
    ...
    def __onCurrentStatusChanged(self, index: int):
        self.__current_status = self.__token_statuses_model.getStatus(index)
    ...
    def __setCurrentStatus(self, status: str | None = None) -> bool:
        if status is None:
            ...
            index: int | None = self.__token_statuses_model.getStatusIndex(status)
            if index is None:
                self.setCurrentIndex(0)
                return False
            else:
    ...
    def setToken(self, token: TokenClass | None = None):
        ...
        self.__token_statuses_model.setToken(token)
...
class ComboBox_InstrumentType(QtWidgets.QComboBox):
    ...
    class InstrumentsTypeModel(QtCore.QAbstractListModel):
        ...
        __ANY_TYPE: str = 'Любой'
        __PARAMETER: str = 'instrument_type'
        ...
        def data(self, index: QtCore.QModelIndex, role: int = ...) -> typing.Any:
            if role == QtCore.Qt.ItemDataRole.DisplayRole:
                row: int = index.row()
                return self.__ANY_TYPE if row == 0 else self.__types[row - 1]
            ...
            if self.__token is None:
                ...
                if db.transaction():
                    ...
                    while types_query.next():
                        self.__types.append(types_query.value(self.__PARAMETER))
...
class ComboBox_Instrument(QtWidgets.QComboBox):
    ...
    def __model_update(decorated_function):
        def wrapper_function(self: ComboBox_Instrument, parameter):
            ...
            if self.__current_item is None:
                self.setCurrentIndex(0)
                self.instrumentsListChanged.emit(
                    self.instruments_model.uids)  # Испускается при изменении списка инструментов.
            else:
...
class GroupBox_InstrumentSelection(QtWidgets.QGroupBox):
    ...
    def __init__(self, tokens_model: TokenListModel, parent: QtWidgets.QWidget | None = None):
        ...
        self.comboBox_status = ComboBox_Status(token=self.token, parent=self)
        ...
        self.comboBox_instrument_type = ComboBox_InstrumentType(token=self.token, status=self.status, parent=self)
        ...
        self.comboBox_instrument = ComboBox_Instrument(token=self.token, status=self.status, instrument_type=self.instrument_type, parent=self)
    ...
    def __onCurrentInstrumentChanged(self, uid: str | None = None):
        if uid is None:
            ...
            if type(self.__current_instrument) is MyBondClass:
                self.bondSelected.emit(self.__current_instrument)
            elif type(self.__current_instrument) is MyShareClass:
                self.shareSelected.emit(self.__current_instrument)
            else:
                raise SystemError('Инструмент не найден!')
